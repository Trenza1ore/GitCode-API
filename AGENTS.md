...
`gitcode-api` is a community-maintained Python SDK for the GitCode REST API. It exposes synchronous and asynchronous `httpx` clients, resource-oriented helpers such as `client.repos` and `client.pulls`, lightweight response models, a small CLI, examples, and Sphinx documentation.
...
Commands mirror synchronous `GitCode` resource methods with the pattern `gitcode-api <resource> <method> ...`. Extra `**params` or `**payload` values can be passed with repeated `--set key=value` flags or `--set-json '{"key": "value"}'`.
