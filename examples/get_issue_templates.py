"""List GitCode issue templates and print one template body.
...
def main() -> None:
    ...
    with GitCode(api_key=config.api_key, owner=config.username, repo=config.repo) as client:
        templates = client.issues.list_templates()
        print(f"repository: {config.username}/{config.repo}")
        print(f"active issue templates (after SDK resolution): {len(templates)}")
        print("")
        ...
        if not templates:
            return
        first = templates[0]
        body = client.issues.get_template(
            path=first.path,
